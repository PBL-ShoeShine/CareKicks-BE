...
class CustomerProfileService {
    ...
    if (Object.keys(customerPayload).length > 0) {
      if (existingCustomer) {
        ...
        const { error: errUpdate } = await supabase
          ...
          .eq('id_user', id_user);
        if (errUpdate) throw new Error('Gagal update customers: ' + errUpdate.message);
      } else {
        ...
        if (errInsert) throw new Error('Gagal insert customers: ' + errInsert.message);
      }
    }
  ...
  async updateNoHp(id_user, no_hp, password) {
    ...
    if (updateCustomerError) {
      console.warn('Peringatan: Gagal sinkronisasi no_hp ke tabel customers:', updateCustomerError.message);
    }
