...
class AlamatService {
  async getAlamat(id_user) {
    ...
    if (error)
      throw new Error("Gagal mengambil daftar alamat: " + error.message);
    return data || [];
  }
  ...
  async setDefault(id_address, id_user) {
    ...
    if (error)
      throw new Error("Gagal mengatur alamat default: " + error.message);
  ...
  async _syncToCustomers(id_user, alamat, latitude, longitude) {
    try {
      ...
      if (existingCustomer) {
        ...
        if (updateError)
          console.error(
            "[AlamatService] Gagal update customers:",
            updateError.message,
          );
      } else {
        ...
        if (insertError)
          console.error(
            "[AlamatService] Gagal insert ke customers:",
            insertError.message,
          );
      }
    } catch (err) {
      console.error(
        "[AlamatService] Terjadi kesalahan saat sinkronisasi:",
        err.message,
      );
    }
  }
