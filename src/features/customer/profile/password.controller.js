...
const verifyOldPassword = async (req, res) => {
  try {
    ...
    const isValid = await passwordService.verifyOldPassword(
      id_user,
      old_password,
    );
    if (!isValid)
    ...
    return res
      .status(200)
      .json({ success: true, message: "Password lama sesuai." });
  } catch (error) {
...
const changePasswordDirect = async (req, res) => {
  try {
    ...
    await passwordService.changePasswordDirect(
      id_user,
      old_password,
      new_password,
    );
...
const changePasswordOtp = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Password berhasil diatur ulang." });
  } catch (error) {
    return res.status(400).json({ success: false, message: error.message });
  }
};
