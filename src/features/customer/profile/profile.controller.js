...
const customerProfileController = {
  ...
  updateProfile: async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, message: 'Profil berhasil disimpan.' });
    } catch (error) {
      return res.status(500).json({ success: false, message: error.message });
    }
  },
  ...
  requestEmailChange: async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, ...result, isWaitingVerification: true });
    } catch (error) {
  ...
  updateNoHp: async (req, res) => {
    try {
      ...
      if (!no_hp || !password) {
        return res.status(400).json({ success: false, message: 'Nomor telepon dan kata sandi wajib diisi.' });
      }

      const result = await profileService.updateNoHp(id_user, no_hp.trim(), password);
  ...
  updateProfilePicture: async (req, res) => {
    try {
      ...
      if (updateDbError) {
        throw new Error('Gagal menyimpan URL ke database: ' + updateDbError.message);
      }
      ...
      if (updateCustomerError) {
        console.warn('Peringatan: Gagal sinkronisasi foto ke tabel customers:', updateCustomerError.message);
      }
