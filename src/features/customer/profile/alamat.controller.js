...
const alamatController = {
  ...
  addAlamat: async (req, res) => {
    try {
      ...
      return res.status(201).json({ success: true, message: 'Alamat berhasil ditambahkan.', data });
    } catch (error) {
  ...
  updateAlamat: async (req, res) => {
    try {
      ...
      const data = await alamatService.updateAlamat(id_address, id_user, req.body);
      return res.status(200).json({ success: true, message: 'Alamat berhasil diperbarui.', data });
    } catch (error) {
      return res.status(400).json({ success: false, message: error.message });
    }
  },
  ...
  setDefault: async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, message: 'Alamat default berhasil diubah.' });
    } catch (error) {
      return res.status(400).json({ success: false, message: error.message });
    }
  },
