...
exports.addPaymentMethod = async (req, res) => {
  try {
    ...
    const { tipe_pembayaran, nama_bank, no_rek, atas_nama, is_default } = req.body;
...
exports.updatePaymentMethod = async (req, res) => {
  try {
    ...
    const data = await paymentService.updatePaymentMethod(idAccount, idShops, payload);
...
exports.toggleStatus = async (req, res) => {
  try {
    ...
    if (typeof is_active !== 'boolean') {
      return res.status(400).json({ success: false, message: 'Status harus berupa boolean (true/false).' });
    }

    const data = await paymentService.updatePaymentMethod(idAccount, idShops, { is_active });
...
exports.uploadQrisImage = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ success: false, message: 'File gambar QRIS tidak ditemukan.' });
    }
    ...
    const data = await paymentService.updatePaymentMethod(idAccount, idShops, { path_qris: publicUrl });
